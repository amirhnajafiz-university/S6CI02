...
def initialize():
    ...
    f.With('age').NewSession('mid').Range([33, 45, 38])
    f.With('age').NewSession('old').Range([40, 58, 48])
    f.With('age').NewSession('veryold').Range([52, 60, 60]).RightValue(1)

    f.NewParam('blood_pressure').NewSession('low').Range([111, 134, 111]).LeftValue(1)
    f.With('blood_pressure').NewSession('medium').Range([127, 153, 139])
    f.With('blood_pressure').NewSession('high').Range([142, 172, 157])
    f.With('blood_pressure').NewSession('veryhigh').Range([154, 171, 171]).RightValue(1)
    ...
    f.With('cholesterol').NewSession('medium').Range([188, 250, 215])
    f.With('cholesterol').NewSession('high').Range([217, 307, 263])
    f.With('cholesterol').NewSession('veryhigh').Range([281, 347, 347]).RightValue(1)

    f.NewParam('heart_rate').NewSession('low').Range([100, 141, 100]).LeftValue(1)
    f.With('heart_rate').NewSession('medium').Range([111, 194, 152])
    f.With('heart_rate').NewSession('high').Range([152, 210, 210]).RightValue(1)

    f.NewParam('ecg').NewSession('normal').Range([0, 0.4, 0]).LeftValue(1)
    f.With('ecg').NewSession('abnormal').Range([0.2, 1.4, 1])
    f.With('ecg').NewSession('hypertrophy').Range([1.4, 1.9, 1.9]).RightValue(1)

    f.NewParam('old_peak').NewSession('low').Range([1, 2, 1]).LeftValue(1)
    f.With('old_peak').NewSession('risk').Range([1.5, 4.2, 2.8])
    f.With('old_peak').NewSession('terrible').Range([2.5, 4.1, 4.1]).RightValue(1)
